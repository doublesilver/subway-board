...
flowchart LR
    ...
    D -->|AI 장애| E
```
- **1차 방어**: `ProfanityFilter` - 한국어 비속어 패턴 매칭 (빠르고 확실)
- **2차 방어**: Google Gemini AI - 문맥 기반 판단 (혐오표현, 폭력, 성적 콘텐츠)
- **Fail-safe**: AI 장애 시에도 1차 필터 통과했으면 서비스 정상 운영

### 13. 코드 품질 개선 (v2.2)
| 개선 항목 | Before | After |
|----------|--------|-------|
| Socket.IO 의존성 | `global.io` 전역변수 | DI 패턴 (`setSocketIO()`) |
| 퇴장 플래그 | `window.__setLeavingManually` | 모듈 스코프 `Map` |
...
│   │   │   └── validator.js            # 입력 검증
│   │   ├── 📂 services/         # 서비스
│   │   │   └── aiService.js       # AI Cleanbot (Gemini)
│   │   ├── 📂 routes/           # API 라우트
│   │   └── 📂 utils/            # 유틸리티
...
| **Rate Limiting** | API 요청 제한 (쓰기: 50회/15분, 읽기: 100회/분) |
| **CORS** | 허용된 도메인만 접근 가능 |
| **XSS 방어** | `xss` 라이브러리로 입력 필터링 |
| **SQL Injection** | Parameterized Query 사용 |
| **콘텐츠 모더레이션** | ProfanityFilter + Google Gemini AI 이중 방어 |
...
gantt
    ...
    section v2.2
    AI Cleanbot 통합     :done, 2026-01-16, 1d
    코드 품질 개선       :done, 2026-01-16, 1d
```
